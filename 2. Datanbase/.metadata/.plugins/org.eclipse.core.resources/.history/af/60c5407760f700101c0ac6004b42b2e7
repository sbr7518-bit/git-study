...
-- 테이블 구조 변경 (컬럼 삭제) -> DROP (column은 생략가능, 보통 컬럼 수정이 더 많음)
ALTER TABLE ddl_alter DROP COLUMN alter_as;


-- ADD CONSTRAINT : 컬럼에 중복된 값이 들어가면 안된다는 제약조건을 걸겠다 UNIQUE(조건 걸 컬럼명); 
ALTER TABLE ddl_alter 
...
-- MySQL에선 지원안됨. 수정하려면 제거 후 다시 추가 (oracle에서 사용가능) 
-- ALTER TABLE ddl_alter 
...
ALTER TABLE ddl_alter 
DROP COLUMN alter_name;

ALTER TABLE ddl_alter 
 ADD alter_as varchar(255);

ALTER TABLE ddl_alter 
 ADD CONSTRAINT alter_name_unique UNIQUE(alter_as);

CREATE TABLE ddl_alter (
	alter_id varchar(255),
	alter_name varchar(255),
	alter_as varchar(255),
	alter_pwd varchar(255)
);
...
------------------------------------------------------------------------
제약조건 (CONSTRAINT)
- NOT NULL (해당 컬럼에 빈 값이 들어올 수 없음을 뜻함)

- UNIQUE (이름은 중복이 가능하다) 

- CHECK (정해진 값을 적용할 때 사용)

- DEFAULT (날짜나 조회수에 주로 사용)  -> 현재 기본값을 넣겠다

- PRIMARY KEY (기본키 : 회원번호, 주문번호 등에 사용 / 반드시 유일(unique)해야하고 NULL이 들어올 수 없음)

- FOREIGN KEY (기본키(PK)를 참조 : 작성자 (회원번호 멤버를 참조), 주문자 등에 사용. NULL 들어갈 수 있음)
    >  PK에 등록된 값을 참조에서 가져오기 때문에 직접 작성 불가
    >  외래어 키. 기본이 되는 테이블이 부모테이블, 참조에서 가져온 값을 작성한 것이 자식 테이블

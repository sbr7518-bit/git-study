...
2. 이상 문제
	...
	- 갱신 이상 : 중복되는 튜플(데이터) 중 일부의 속성(컬럼)만 갱신 시킴으로써 정보의 모순성이 
	            발생하는 경우


3. 정규화를 너무 많이하면 생기는 단점
	- 릴레이션 분해가 많이 될 수록 JOIN을 자주 사용하게 되어 SELECT 쿼리의 속도가 
	  상대적으로 느려기지 때문에 적정선에서 해야 함

4. 데이터베이스 반정규화
	- 성능 향상을 목적으로 정규화된 데이터 모델을 정규화 원칙에 벗어나게 하는 것을 의미
	- 요약 : 쪼개진 테이블을 다시 합침. 
	- 반정규화 하는 이유 : SELECT를 자주하는데 성능상 문제가 발생하는 경우(조회가 느림 등)
	- 단점 : 이상 문제가 발생할 수 있다 (데이터 삭제 및 갱신 오류)

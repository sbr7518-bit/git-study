...
//	  4. 실수형 
	 float pi = 3.14F;				//4byte  보통 7~8자리까지 보여줌
	 double gravity = 9.80665;      //8byte       15자리까지 
...
//	 실수형은 메모리 설계상 양수보다 더 넓은 범위를 사용하기 때문에 자동 형변환이 가능
...
//	 8. 강제 형변환 (명시적 형변환) 
//       - 개발자가 직접 'int'타입으로 형변환을 해줘야 하고 하지않을 경우 컴파일 에러 발생
//	 	 - 범위가 큰 자료형 -> 범위가 작은 자료형
//	 	 - 주의 : 큰 자료형에서 작은 자료형으로 변환하기 때문에 데이터 손실 발생 할 수 있음

	 long lNumber = 10000000000L;
	 int iNumber = (int)lNumber; // 8byte를 4byte로 강제 변경 
	                             // (큰 애를 작은애한테 넣을경우 데이터 손실 발생)
